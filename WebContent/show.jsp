	...
	<form name="Menu" action="./SelectCategoryServlet" method="post">
		<ul class="mapmenu">
			...
			<li><a id="prefecture_menu"> ${prefecture.name} </a>
				<ul>
					<c:forEach items="${prefectures}" var="prefecture">
						<li id="prefecture"><a onclick="changeText('${prefecture.name}','prefecture_menu'); map.clearFeatures(); setInputValue('${prefecture.id}','prefecture_input');">
							${prefecture.name}
						</a></li>
					</c:forEach>
				</ul>
			...
			<li><input type="hidden" name="action" value="list">
				...
				style="text-decoration: none"> 表 示 </a></li>
		</ul>
	</form>
